...
void main ()
{
    ...
    // Random X position
    positionVec4.x += rand(vec2(nid)) * u_resolution.x - u_resolution.x * 0.5;
    
    // Move to the top
    positionVec4.y -= u_resolution.y * 0.5;
  
    // Slowly falling
    float y = time * (1.0 + rand(vec2(nid + 0.1) ) );
  
    // Random Y offset
    y += rand(vec2(nid + 0.2)) * u_resolution.y;
  
    // Reset at the bottom
    y = mod(y, u_resolution.y);
